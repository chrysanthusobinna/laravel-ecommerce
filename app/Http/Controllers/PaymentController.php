...
class PaymentController extends Controller
{
    ...
    //                     // return redirect()->route('product.list')->withErrors('Order verification failed');
    ...
    //                 return redirect()->route('product.list')->withErrors("There was an issue processing your payment. Please try again.");



    //         } catch (Exception $e) {
    //             $error_msg  =  $e->getMessage();
    //             return redirect()->route('product.list')->withErrors($error_msg);
    //         }
    //     } else {
    //         return redirect()->route('product.list')->withErrors('Session ID not found!');
    //     }
    // }
    ...
    public function paymentSuccess(Request $request)
    {
        ...
        return match ($this->provider) {
            ...
            default   => redirect()->route('product.list')->withErrors('Unsupported payment provider.'),
        };
    }
    ...
    private function handleStripeSuccess(Request $request)
    {
        ...
        try {
            ...
            if (!$order_no) {
                return redirect()->route('product.list')->withErrors('No order reference found.');
            }
    ...
    //         return redirect()->route('product.list')->withErrors('Your cart is empty. Please add items to your cart before checking out.')->send();
    //     }
    // }
    ...
    //         return redirect()->route('product.list')->withErrors('We could not retrieve your customer details. Please try again or contact support if the issue persists.')->send();
    //     }
    // }
    ...
    //         return redirect()->route('product.list')->withErrors('We could not retrieve your delivery details. Please try again or contact support if the issue persists.')->send();
    //     }
    // }
    ...
    protected function checkOrderNo()
    {
        if (!session()->has('order_no')) {
            //return redirect()->route('product.list')->withErrors('We could not retrieve your order number. Please try again or contact support if the issue persists.')->send();
            return redirect()->route('product.list')->send();
        }
    }
