...
class CustomerController extends Controller
{
    ...
    // Clear cart and redirect to product list
    public function clearCart(Request $request)
    {
        // Use the CartTrait to clear the cart
        $response = $this->clearCart($request);
        
        if ($response->getData(true)['success']) {
            return redirect()->route('product.list')
                ->with('success', 'Your cart has been cleared successfully.');
        } else {
            return redirect()->route('product.list')
