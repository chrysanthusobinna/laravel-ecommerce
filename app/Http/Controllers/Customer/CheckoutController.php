...
class CheckoutController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->vat_percentage = $order_settings->vat_percentage ?? 0;
        ...
        if ($request->mode === 'saved') {
            $deliveryAddressId = (int) $request->saved_address_id;
                ...
                if ($distance_in_miles > $this->distance_limit_in_miles) {
                    $error_message = "We're sorry! We can only deliver within {$this->distance_limit_in_miles} miles. You can still place your order as a walk-in at our Store located at {$this->companyAddress->full_address}. We look forward to serving you!";
    ...
    public function review()
    {
        ...
        if (!session()->has($this->cartkey)) {
            return redirect()->route('product.list')->withErrors('Your cart is empty. Please add items to your cart before checking out.');
        }
        ...
        $total = $subtotal + $delivery_fee + $vat_amount;

        return view('main-site.checkout-review', compact('user', 'cart_items', 'delivery_fee', 'subtotal', 'vat_amount', 'vat_percentage', 'total'));
    }
    ...
    public function proccessCheckout(Request $request)
    {
        ...
        foreach ($cart_items as $cart_item) {
            ...
            if (!empty($cart_item['size_name'])) {
                $productName .= ' - ' . $cart_item['size_name'];
            }
